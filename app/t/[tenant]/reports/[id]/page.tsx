...
export default async function ReportPage({ params, searchParams }: PageProps) {
  ...
  const { meta } = resolved;
  const boundary = parseBoundary(searchParams.boundary);
  const stagesAvailable = hasStages(resolved.report);
  // If the report doesn't carry stages, force grave (the only number we have).
  const effectiveBoundary = stagesAvailable ? boundary : "grave";
  const report = applyBoundary(resolved.report, effectiveBoundary);
    ...
    <div className="bg-white">
      ...
      <footer className="border-t border-slate-200 py-8">
        <div className="max-w-7xl mx-auto px-6 text-xs text-slate-500 flex flex-wrap items-center justify-between gap-3">
          <div>
            Generated by Scope · v3.0 engine on the Grounded LCA assumption playbook · ISO 14040/14044 · Ellen MacArthur MCI · Boundary: {BOUNDARY_LABEL[effectiveBoundary]}
          </div>
